...
| `http://localhost:8080/#bline` | `currentView === 'catalog'` | **Design reference (internal)** | `<BLineCatalogSection />` | Dev only (`?dev=1`) | The 12 B—Line S.r.l. design pieces the layout was modelled on — not SmartGift products; the nav link is hidden outside dev mode and the page is labelled as a reference. |
